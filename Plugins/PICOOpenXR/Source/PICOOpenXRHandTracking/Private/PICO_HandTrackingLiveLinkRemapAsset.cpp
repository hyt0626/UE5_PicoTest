...
#include "PICO_HandTrackingLiveLinkRemapAsset.h"
#include "PICO_HandTracking.h"

#include "BonePose.h"
#include "Roles/LiveLinkAnimationTypes.h"

#define LOCTEXT_NAMESPACE "HandTrackingLiveLinkRemapAssetPICO"

UHandTrackingLiveLinkRemapAssetPICO::UHandTrackingLiveLinkRemapAssetPICO(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

#if WITH_EDITORONLY_DATA
void UHandTrackingLiveLinkRemapAssetPICO::PostInitProperties()
{
	...
	if (HandTrackingBoneNameMap.Num() == 0)
	{
		...
		{
			FName BoneName = FHandTrackingPICO::ParseEOpenXRHandKeypointEnumName(EnumPtr->GetNameByValue(Keypoint));
...
void UHandTrackingLiveLinkRemapAssetPICO::BuildPoseFromAnimationData(float DeltaTime, const FLiveLinkSkeletonStaticData* InSkeletonData, const FLiveLinkAnimationFrameData* InFrameData, FCompactPose& OutPose)
{
...
FName UHandTrackingLiveLinkRemapAssetPICO::GetRemappedBoneName(FName BoneName) const
{
...
FTransform UHandTrackingLiveLinkRemapAssetPICO::GetRetargetedTransform(const FLiveLinkAnimationFrameData* InFrameData, int TransformIndex) const
{
	check(InFrameData);
